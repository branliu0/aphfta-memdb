...
def get_direct_fields_from_model(model_class):
    direct_fields = []
    all_fields_names = model_class._meta.get_all_field_names()
    if model_class.__name__.lower() == 'facility':
        all_fields_names = filter_facility_fields(all_fields_names)
    for field_name in all_fields_names:
        field = model_class._meta.get_field_by_name(field_name)
...
def filter_facility_fields(fields):
    fields_to_remove = ['id']
    filtered_fields = [field for field in fields if not field in fields_to_remove]
